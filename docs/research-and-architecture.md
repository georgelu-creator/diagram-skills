...
Research date: 2026-08-27. The goal was to reuse proven diagram infrastructure and keep DiagramSpec-specific code focused on its information model, quality contract, and Agent workflow.
...
## What DiagramSpec implements

DiagramSpec supplies the pieces that are product-specific rather than generic infrastructure:
...
- CSV column mapping into the DiagramSpec model;
...
- [Azure icon guidance](https://learn.microsoft.com/en-us/azure/architecture/icons/) places product names near icons and prohibits distorting product marks. DiagramSpec therefore uses an original, consistent line-icon vocabulary rather than pretending generic glyphs are vendor logos.
...
- [diagrams.net](https://www.drawio.com/docs/manual/import/import-formats/) already imports Mermaid and specially formatted CSV, supports multi-page files, and exposes an embed protocol. It remains a strong future interoperability target. It was not embedded as the core editor because its XML/page model would make DiagramSpec JSON a secondary source of truth.
- Mermaid alone is excellent for source-first diagrams but does not provide the direct node inspector, multi-view workspace, stable DiagramSpec semantics, or cross-format JSON contract required here.
