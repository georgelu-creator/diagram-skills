...
import { parseWorkspace, removeDiagramNodes, serializeWorkspace, validateWorkspaceUpdate, type VisualSpecView } from "./model";
...
describe("workspace schema", () => {
  ...
  it("rejects duplicate node, group, and lane identifiers", () => {
    ...
    const nodeView = duplicateNode.views[0] as VisualSpecView;
    ...
    const groupView = duplicateGroup.views[0] as VisualSpecView;
    ...
    const laneView = duplicateLane.views[0] as VisualSpecView;
  ...
  it("rejects unknown or empty boundaries", () => {
    ...
    (unknownGroup.views[0] as VisualSpecView).nodes[0].group = "missing";
    ...
    (emptyLane.views[0] as VisualSpecView).lanes.push({ id: "empty", label: "Empty", order: 9 });
  ...
  it("rejects unsafe links and unmarked cycles but permits feedback", () => {
    ...
    (unsafe.views[0] as VisualSpecView).nodes[0].link = "javascript:alert(1)";
    ...
    (cyclic.views[0] as VisualSpecView).edges.push({ source: "brief", target: "intent", kind: "primary" });
  ...
  it("enforces the same diagram-type grammar used by the CLI", () => {
    ...
    const architectureView = architecture.views[0] as VisualSpecView;
    ...
    const decisionView = decision.views[0] as VisualSpecView;
  ...
  it("returns an explicit failed update instead of accepting invalid workspace state", () => {
    ...
    (invalid.views[0] as VisualSpecView).nodes[0].lane = "missing";
  ...
  it("blocks deletion of the final node or the final member of a boundary", () => {
    ...
    const overview = sampleWorkspace.views[0] as VisualSpecView;
